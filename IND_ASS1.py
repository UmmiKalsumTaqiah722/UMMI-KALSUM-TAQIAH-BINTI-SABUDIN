...
def collect_data():
    ...
    ID = Member_ID_entry.get()
    YoB= int(Member_YoB_entry.get())

    # Calculate the age. This will be derived from your selection (Year of birth).
    Age = 2023 - YoB

    # To insert your Data to your database, As for this example, you have 4 attributes. (2 Attributes from your selection (Name, ID Number, Gender, Year of born) and another attributes that derived from your attributes (age))
    sql = "INSERT INTO Member_Information (Name, ID_Number, Gender, Year_of_Born, Age) VALUES (%s, %s, %s, %s, %s)"
    val = (Name, Gender, ID, YoB, Age)
    ...
    output_label.config(text=f"Year of born: {YoB}, Age: {Age}")
...
Member_Name_label = tkinter.Label(Member_Info, text="Full Name")
